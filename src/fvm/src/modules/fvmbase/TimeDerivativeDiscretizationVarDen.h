...
#ifndef _TIMEDERIVATIVEDISCRETIZATIONVARDEN_H_
#define _TIMEDERIVATIVEDISCRETIZATIONVARDEN_H_
...
class TimeDerivativeDiscretizationVarDen : public Discretization
{
public:
  ...
  TimeDerivativeDiscretizationVarDen(const MeshList& meshes,
                               const GeomFields& geomFields,
                               Field& varField,
                               Field& varN1Field,
                               Field& varN2Field,
                               const Field& densityField,
			       const Field& densityN1Field,
                               const T_Scalar dT) :
      ...
      _densityN1Field(densityN1Field),
      _dT(dT)
      {}
                  ...
                  MultiField& xField, MultiField& rField)
  {
    ...
    const TArray& densityN1 =
      dynamic_cast<const TArray&>(_densityN1Field[cells]);
        ...
        if (_geomFields.volumeN1.hasArray(cells))
	{
	    ...
	    for(int c=0; c<nCells; c++)
	        ...
	        const T_Scalar rhoVbydT = density[c]*cellVolume[c]/_dT;
		const T_Scalar rhobydT = density[c]/_dT;
		const T_Scalar rhobydTN1 = densityN1[c]/_dT;
	        rCell[c] -= rhobydT*cellVolume[c]*x[c] 
                                     - rhobydTN1*cellVolumeN1[c]*xN1[c];
        ...
        else
	{
            ...
            {
                const T_Scalar rhoVbydT = density[c]*cellVolume[c]/_dT;
		const T_Scalar rhoVbydTN1 = densityN1[c]*cellVolume[c]/_dT;
                rCell[c] -= rhoVbydT*x[c]- rhoVbydTN1*xN1[c];
		diag[c] -= rhoVbydT;
